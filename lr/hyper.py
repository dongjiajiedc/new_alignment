...
def multi_train(times,dataset,dataloader,learning_rate,similarities,epoches1):
    best_cost = np.inf;
    best_model = None;
    for i in range(times):
        ...
        if(cost<best_cost):
            ...
            best_model = model; 
        print('---');
        print("{} best_cost: {}".format(i,best_cost));
        print('---')
    return best_model;

def get_Hyper_tree(data_path,start,end,label,epoches1,epoches2,meta_list,model_path=None,save_path='./',learning_rate =0.0005, mst1 = False,multi = 10):
    """
    ...
    x, y_true, similarities = load_data(data_path,start,end,label)
    # similarities = np.array([[5,4,4,3,3],[4,5,3,4,4],[4,3,5,2,2],[3,4,2,5,3],[3,4,2,3,5]],dtype=np.float);
    # similarities = np.array([[0.          , 0.2556532864, 0.9144031133, 0.9521954098,
    #     0.451254055 ],
    #    [0.2556532864, 1.          , 0.6586227819, 0.8899632871,
    #     0.8905571879],
    #    [0.9144031133, 0.6586227819, 2.          , 0.9935673629,
    #     0.9732647911],
    #    [0.9521954098, 0.8899632871, 0.9935673629, 3.          ,
    #     0.5934113296],
    #    [0.451254055 , 0.8905571879, 0.9732647911, 0.5934113296,
    #     4.          ]])
    similarities =np.array([[1.          , 0.6993498232, 0.0791033551, 0.416892978 ,
    ...
    similarities = np.array([[1.9995527864, 1.7542556889, 0.9892064807, 1.6836552229,
    ...
    similarities = np.array([[1.          , 0.93866201  , 0.7473569309, 0.921007946 ,
        0.7474329017, 0.9129864423, 0.7473914377],
       [0.93866201  , 1.          , 0.7470278586, 0.9499874411,
        0.7470538957, 0.937946548 , 0.7470390948],
       [0.7473569309, 0.7470278586, 1.          , 0.7470007242,
        0.9767343919, 0.7470497279, 0.9841489589],
       [0.921007946 , 0.9499874411, 0.7470007242, 1.          ,
        0.7469936452, 0.9631999341, 0.746996526 ],
       [0.7474329017, 0.7470538957, 0.9767343919, 0.7469936452,
        1.          , 0.7470247197, 0.9830269347],
       [0.9129864423, 0.937946548 , 0.7470497279, 0.9631999341,
        0.7470247197, 1.          , 0.7470371731],
       [0.7473914377, 0.7470390948, 0.9841489589, 0.746996526 ,
        0.9830269347, 0.7470371731, 1.          ]])
    ...
    # dataset = HCDataset(x, y_true, similarities, num_samples=5*4*3*100)
    dataset = HCDataset(x, y_true, similarities, num_samples=2100*10)
    ...
    if(model_path==None or os.path.exists(model_path)==False):
        ...
        model = multi_train(multi,dataset,dataloader,learning_rate,similarities,epoches1);
    else:
